...
Daily minimum temperature is derived from other debiased fields rather than bias-corrected directly, and the fine-resolution `tasmax`/`tasmin` pair is reconciled after disaggregation. [Scientific approach](scientific-approach.md) explains why the pipeline works this way; this section covers how the two behaviours are wired into the stages.

The derivation runs in the dedicated stage variants `fit_historical_tasmin` and `transform_scenario_tasmin`, which read the `debiased_coarse` `tasmax` and `dtr` groups written by `fit_historical` and `transform_scenario` (see [Managing the Cache](../how-to/manage-cache.md)). The reconstruction helper (`derive_tasmin`) requires its two inputs to share an identical time axis and raises if they do not, so a truncated or misaligned `dtr` fails loudly instead of silently NaN-filling the result (issue #363).

The reconcile step (`reconcile_temperature_extremes`) runs once both fine fields exist, swapping the offending cells and rewriting both corrected fields (issue #331). The swap is NaN-safe and structurally monotone, and the `bcsd validate-output` gate blocks any run whose stored output still contains an inversion.
