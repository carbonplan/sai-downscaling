...
Comparing full global output on every change would be prohibitively expensive, so the routine per-pull-request check runs over a small South Africa subset instead. Because BCSD's regridding and spatial disaggregation use neighboring cells, that subset is produced over a small halo beyond the region of interest, and the notebook trims both runs back to the region of interest before comparing — otherwise boundary cells, computed with a truncated neighborhood in the regional run, would masquerade as real changes. To make the trimmed candidate comparable to the global baseline, the notebook subsets the global snapshot to the candidate's extent with a plain per-leaf `.sel` — a straightforward selection, not a coordinate reconciliation, so if the two grids do not actually line up the selection fails loudly rather than papering over the mismatch. The comparison engine itself does no alignment: `compare_runs` diffs two stores as-is, each read on its own icechunk branch, and the verdict comes from `xarray.testing.assert_allclose`, which checks dimensions, coordinates, and values together. The cheap South Africa check and the full global check therefore share one comparison engine and one tolerance policy; only the notebook's halo trim and subset step differ.

One limitation of the windowed proxy is worth flagging. The bias correction fits a per-variable distribution: `tas`, `tasmax`, and `tasmin` use a Gaussian — a closed-form, numerically stable fit — but `hurs`, `rsds`, and `dtr` use parametric fits (a beta distribution, with Weibull and Gumbel tails) whose iterative maximum-likelihood estimates are sensitive to tiny input perturbations. Windowing the domain changes the dask and regrid reduction order, which perturbs the inputs at the floating-point level; the Gaussian variables absorb that within tolerance, but the parametric variables can amplify it into large, cell-scale differences even in the interior. As a result `hurs`, `rsds`, and `dtr` diff spuriously in a South-Africa-versus-global comparison, with no real change in the pipeline — a wider halo does not help, because the differences are interior, not at the boundary. Judge those three from the full global comparison (`mode = "global"`), where both runs are global and feed identical inputs to the fits; the cheap South Africa proxy is a reliable regression signal for the numerically stable variables (`tas`, `tasmax`, `tasmin`, `pr`).
