...
def vcpus(vm_type: str) -> int:
    ...
    try:
        ...
        raise KeyError(
            f"Unknown instance type {vm_type!r}; add it to saidownscale.cost.INSTANCE_VCPU"
        ) from None
...
def burn_rate_per_hour(vm_type: str, n_tasks: int, executor: str) -> float:
    ...
    try:
        ...
        raise KeyError(
            f"Unknown executor {executor!r}; add it to saidownscale.cost.RATE_PER_VCPU_HOUR"
        ) from None
