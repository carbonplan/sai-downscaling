"""
Configuration classes for downscaling runs.
            ...
            for old, new in cls._RENAMED_KEYS.items():
                if old in data:
                    raise ValueError(
                        ...
                        f"BCSD/QDMSD selector on DownscalingConfig. Leaving '{old}' here would "
...
class DownscalingConfig(pydantic_settings.BaseSettings):
    """
    Run-identity configuration for downscaling pipeline.

    Captures the parameters that uniquely identify a downscaling run: model, variable,
    ...
    # DownscalingConfig, since those silently mapped variables to the wrong method (issue #423).
    ...
    # Keys that look like they belong here but do not: ones that moved off DownscalingConfig,
    ...
    _REJECTED_KEYS: ClassVar[dict[str, str]] = {
        ...
        "debias_approach": (
            "'debias_approach' moved from DownscalingConfig onto VariableConfig. Set "
        ...
        "variable_overrides": (
            ...
            "DownscalingConfig field. It only takes effect in a matrix config, i.e. one with "
...
class PipelineOptions(pydantic_settings.BaseSettings):
    """
    Operational settings for the downscaling pipeline.
...
# 3. Regional subset
subset_config = DownscalingConfig(
    gcm="UKESM1-1-LL",
    variable="tasmax",
    ensemble_member=0,
    scenario="ssp245",
...
custom_config = DownscalingConfig(
    gcm="UKESM1-1-LL",
    variable="tas",
    ensemble_member=2,
    scenario="ssp245",
