"""
Runs saidownscale.qa_flags.run_step2 as a standalone script, so it can be submitted as a
...
script:
    ...
    import coiled
    coiled.batch.run(
        command=["python", "-m", "saidownscale.run_qaflag_step2"],
        name="qa-flags-step2",
...
or run directly on any machine with Coiled credentials configured:
    ...
    >> uv run python src/saidownscale/run_qaflag_step2.py
...
# including the one inside saidownscale.qa_flags -- hence importing that only after this line

from saidownscale.qa_flags import run_step2  # noqa: E402 -- after matplotlib.use("Agg") above
