...
Distinct from :mod:`saidownscale.qa_checks`, which operates on in-memory arrays during
pipeline execution.
"""
...
from saidownscale.config import GROUP_TO_SCENARIO
...
def _baseline_member(
    ...
    Resolved through :mod:`saidownscale.lineage`, never guessed. Returns ``None`` when the lineage table
...
def enumerate_scenario_comparisons(
    ...
    *,
    ...
    The baseline member comes from :func:`saidownscale.lineage.resolve_member_lineage`, which is what makes
    ...
    gcm : str
        GCM name as registered in :mod:`saidownscale.lineage`.
