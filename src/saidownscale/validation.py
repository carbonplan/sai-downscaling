...
from saidownscale.bcsd_config import BCSDConfig, VariableName
...
# from time_bnds via saidownscale.utils.decode_time_from_bounds, which is where that convention is
# documented in full, and TIME_RANGE in saidownscale.input_data.cesm2_waccm clamps what remains.
...
def check_config_time_domain(config: BCSDConfig) -> CheckResult:
    ...
    :func:`saidownscale.downscaling_utils.detrend` for the surrounding years.
...
class DatasetValidator(pydantic.BaseModel):
    ...
    def check_lineage_member_availability(self) -> CheckResult:
        ...
        """
        from saidownscale.lineage import get_lineage_entries
