...
class BCSDConfig(pydantic_settings.BaseSettings):
    ...
    def make_config_for_variable(self, variable: str) -> BCSDConfig:
        ...
        Rebuilt from the full field set rather than an explicit field list, so no future
        field (e.g. ``obs_dataset``) is silently dropped for the sibling. ``variable_config``
        is excluded from the dump and rebuilt explicitly: the sibling gets its own
        per-variable defaults but inherits this run's ``debias_approach``, matching the
        pre-move behavior.
        """
        ...
        data["variable_config"] = VariableConfig.for_variable(variable).model_copy(
