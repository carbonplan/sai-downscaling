...
class TestBCSDRunDataProperties:
    ...
    def test_obs_calls_load_with_obs_path(self, run):
        ...
        with self._patch_load(fake_ds) as mock_load:
            ds = run.obs
        mock_load.assert_called_once_with(run._cache.obs_loc.store_path)
    ...
    def test_historical_calls_load_with_historical_path(self, run):
        ...
        with self._patch_load(fake_ds) as mock_load:
            ds = run.historical
        mock_load.assert_called_once_with(run._cache.historical_loc(run._hist_member).store_path)
        assert ds is fake_ds
    ...
    def test_scenario_calls_load_with_scenario_path(self, run):
        ...
        with self._patch_load(fake_ds) as mock_load:
            ds = run.scenario
        mock_load.assert_called_once_with(run._cache.scenario_loc.store_path)
        assert ds is fake_ds
